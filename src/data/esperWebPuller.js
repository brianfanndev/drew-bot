...
import { playDislyteSelectors as selectors, playDislyteConstants as constants } from "./playDislyteConstants.js";

export const getEsperUrlList = () => {
  const response = await axios.get(constants.esperListUrl);
  const doc = (new JSDOM(response.data)).window.document;

  return doc.querySelectorAll(selectors.esperListNode).map(x => x.getAttribute("href"));
}
